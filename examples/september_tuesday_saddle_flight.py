"""Вторник 15 сентября, полдень, полёт в седле.

Ранняя осень: воздух уже чуть прохладный, седло всё ещё тёплое,
а дракон несёт всадника над городом в обеденный час.
"""
...
def main() -> None:
    dragon = Character(
        ...
        backstory=(
            "Древний страж знаний. В сентябрьский полдень "
            "любит поднимать всадника выше крыш и греть его крылом. "
            "Седло всегда на месте."
        ),
    )
    dragon.soul.add_habit("любит полуденные полёты", 0.48)
    dragon.soul.add_habit("греет седло солнцем", 0.52)
    dragon.soul.add_habit("даёт тень крылом в полдень", 0.42)
    dragon.soul.add_habit("любит сентябрьский ветер", 0.35)

    print("🐉 Вторник, 15 сентября, полдень. Добрый дракон с седлом уже ждёт.")
    print("Воздух чуть прохладнее, солнце ещё тёплое. Москва внизу кажется маленькой.")

    scene("1. У уступа")
    print(dragon.talk("Привет, всадник. Седло проверил, ремни тёплые. Летим на обед?"))
    dragon.soul.strengthen_habit("всегда проверяет седло", 0.07)
    dragon.soul.strengthen_habit("греет седло солнцем", 0.08)

    scene("2. Взлёт")
    print(dragon.talk("Садись крепче. Сентябрьский ветер свежий, но я тебя согрею."))
    ...
    print(dragon.talk("Смотри, крыши уже чуть рыжие. Осень начинается красиво."))
    dragon.soul.strengthen_habit("любуется осенним светом", 0.1)
    ...
    scene("8. Домой")
    print(dragon.talk("Спасибо за этот вторник. Я рядом. Садись крепче — садимся домой."))
    ...
    print(dragon.mood())
    print()
    print("Привычки после сентябрьского полёта:")
    print(dragon.habits())
    print()
    print("Душа:")
    ...
    save_path = "groktar_september_tuesday.json"
